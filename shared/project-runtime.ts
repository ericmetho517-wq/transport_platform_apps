...
function dashboard(app: TransportApp): string {
  ...
  return shell(app, `<main class="dashboard-grid">
    ...
    <section class="list-card"><h2>Latest records</h2>${["Development road","Urban expansion","Agricultural zone","Service center","Industrial cluster"].map((name,i)=>`<button class="record"><span class="dot"></span><span>${name}<small>Record ${String(i+1).padStart(2,"0")}</small></span><b>›</b></button>`).join("")}</section>
  </main>`);
}

function experience(app: TransportApp): string {
  return shell(app, `<main class="experience"><nav class="side-nav"><button class="active">Overview</button><button>Land use</button><button>Indicators</button><button>Projects</button><button>Reports</button></nav><section class="experience-content"><div class="hero"><div><span>Integrated spatial experience</span><h2>${esc(app.title)}</h2><p>Explore project indicators, land-use patterns and transport assets through one responsive workspace.</p><button class="primary">Explore the map</button></div><div class="hero-stat"><strong>78</strong><span>connected applications</span></div></div>${mapPanel(app)}<div class="feature-row"><article><b>01</b><h3>Spatial overview</h3><p>Inspect the project road and surrounding development zones.</p></article><article><b>02</b><h3>Land indicators</h3><p>Compare land-use classes and development opportunities.</p></article><article><b>03</b><h3>Project evidence</h3><p>Review locally migrated datasets and public metadata.</p></article></div></section></main>`);
}

function story(app: TransportApp): string {
  return shell(app, `<main class="story"><section class="story-hero"><div class="eyebrow">Transport development story</div><h2>${esc(app.title)}</h2><p>A guided narrative connecting infrastructure, geography and development indicators.</p><span>Scroll to explore ↓</span></section><section class="story-section"><div><span>01 · Context</span><h3>The road and its region</h3><p>The project is presented as a sequence of spatial chapters. Local imagery, maps and statistics can be added to the project data folder.</p></div>${mapPanel(app)}</section><section class="story-quote"><blockquote>Transport infrastructure reshapes access, investment and the geography of opportunity.</blockquote></section><section class="story-section reverse"><div><span>02 · Evidence</span><h3>Reading land-use change</h3><p>Indicator panels combine agricultural, urban and industrial land patterns with the project alignment.</p></div><div class="story-visual"><i></i><i></i><i></i><i></i></div></section></main>`);
}
