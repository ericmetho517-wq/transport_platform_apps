...
root.innerHTML = `<div class="platform-shell" dir="rtl">
  ...
  <section id="applications" class="catalog"><div class="section-title"><div><span class="section-kicker">دليل التطبيقات</span><h2>استعرض جميع المشروعات</h2><p>ابحث بالعنوان أو القطاع، ثم صفِّ النتائج حسب النوع واللغة.</p></div><div class="catalog-controls">
    <label><span>بحث</span><input id="app-search" placeholder="ابحث بالعنوان أو القطاع"/></label>
  ...
  </div></div><p id="filter-summary" class="filter-summary" aria-live="polite"></p><div id="app-grid" class="app-grid"></div></section>
...
const render = () => {
  ...
  grid.innerHTML = visible.map((app, index) => `<a class="app-card" href="./projects/${app.slug}/index.html" dir="${app.direction}"><span class="card-number">${String(index + 1).padStart(2, "0")}</span><span class="card-type">${typeLabels[app.type] || app.type}</span><span class="card-icon" aria-hidden="true">${app.type === "Dashboard" ? "▦" : app.type === "StoryMap" ? "◇" : app.type === "Web AppViewer" ? "⌖" : app.type === "Experience" ? "◫" : "▥"}</span><h3>${app.title}</h3><p>${app.category}</p><span class="open">${app.language === "en" ? "Open application" : "فتح التطبيق"} <b>${app.direction === "ltr" ? "→" : "←"}</b></span></a>`).join("") || `<p class="empty">لا توجد تطبيقات مطابقة / No matching applications.</p>`;
};
